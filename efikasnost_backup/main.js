function Main(){
  ...
  this.catchEvents = function(elem){
    if(typeof elem === 'undefined'){
      $('input').on('input', function(){ calc.update(); });
      $('select').change(function(){ calc.update(); });
    }
    else{
      elem.find('input').on('input', function(){ calc.update(); });
      elem.find('select').change(function(){ calc.update(); }); 
    }
  }
  ...
  */
  ...
  this.toDecimalString = function(input){
    ...
    var num = split[0], dec = '';
    if(split.length == 2)
      dec = '.' + split[1].substr(0, parseInt($('#brojDecimala').val()));
    return num + dec; 
  }
